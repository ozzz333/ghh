...
// Canvas App.js source (complete functional implementation from earlier)
export default function ParlayBuilder() {
  ...
  const RANGE_LIMITS = { narrow: 0.01, wide: 0.30 };
  ...
  const addLeg = () => {
    ...
    const probability = calculateWinProbability(
      selectedAsset, lowerBound, upperBound, timeframeHours, livePrice
    );
            ...
            <ul>
              {legs.map((leg, idx) => (
                <li key={idx} className="flex justify-between items-center">
                  ...
                  <Button onClick={() => removeLeg(idx)}>Remove</Button>
                </li>
              ))}
            </ul>
